...
class CategoryTest @Autowired constructor(
    ...
    fun `getCategoryList should return all categories successfully`() {
        ...
        val category1 = categoryService.addCategoryToUser(user.id, "category1", "#FFFFFF")
        val category2 = categoryService.addCategoryToUser(user.id, "category2", "#000000")

        val categories = categoryService.getCategoryList(user.id)
    ...
    fun `getCategoryById should return category successfully`() {
        val user = userRepository.save(createUser())
        val category = categoryService.addCategoryToUser(user.id, "category1", "#FFFFFF")

        val foundCategory = categoryService.getCategoryById(user.id, category.categoryId)
    ...
    fun `getCategoryById should throw exception when category not found`() {
        val user = userRepository.save(createUser())
        categoryService.addCategoryToUser(user.id, "category1", "#FFFFFF")

        assertThatThrownBy {
            categoryService.getCategoryById(user.id, "notFoundCategoryId")
    ...
    fun `updateCategory should update category successfully`() {
        ...
        val updatedCategory = categoryService.updateCategory(user.id, category.categoryId, "category2", "#000000")
    ...
    fun `removeCategoryFromUser should remove category successfully`() {
        ...
        val removedCategory = categoryService.removeCategoryFromUser(user.id, category.categoryId)
